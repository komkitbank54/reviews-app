...
const ScoreBadge = memo(function ScoreBadge({
  ...
  return (
    <span
      className={
        "inline-flex items-center gap-1 rounded-lg border border-white/10 bg-zinc-900/70 backdrop-blur px-2.5 py-1 text-[13px] md:text-[14px] text-zinc-200 " +
        className
      }
    >
...
function MobileGifHint({
  ...
  const pos = placement === "tr" ? "top-10 right-2" : "bottom-2 right-2";
  return (
    <button
      type="button"
      onClick={(e) => {
...
/* ----------------------------- Modal (Guide) ----------------------------- */
function useKeydown(fn: (e: KeyboardEvent) => void) {
  useEffect(() => {
    const h = (e: KeyboardEvent) => fn(e);
...
function GuideModal({
  ...
  // ล็อกสกอร์ลพื้นหลังตอนเปิดโมดัล
  useEffect(() => {
    if (!open) return;
      ...
      {open && (
        ...
        >
          ...
          <motion.div
            ...
            aria-modal="true"
            className="
              relative z-[101] w-full md:max-w-xl
          ...
          >
            {/* Header (ติดบน) */}
            <div className="shrink-0 p-4 md:p-5 border-b border-white/10 flex items-center justify-between">
              <h3 className="text-[16px] md:text-[18px] font-semibold text-zinc-100">
                หากสินค้าไม่ขึ้น (ภายในแอป TikTok)
              </h3>
              <button
                onClick={onClose}
                className="inline-flex h-8 w-8 items-center justify-center rounded-lg hover:bg-white/10 text-zinc-300"
                aria-label="ปิด"
              >
                <X size={18} />
              </button>
            ...
            <div
              className="
                grow overflow-y-auto overscroll-contain
                p-4 md:p-5 space-y-4
                [webkitOverflowScrolling:touch]
              "
            >
              <p className="text-[13px] md:text-[14px] text-zinc-400">
                ให้เปิดลิงก์นี้ด้วยเบราว์เซอร์ภายนอกตามขั้นตอนด้านล่าง (บางเครื่องหน้า
                “คูปองพิเศษ” ของ TikTok อาจแสดงข้อผิดพลาด)
              </p>

              <ol className="space-y-4">
                <li className="rounded-xl border border-white/10 overflow-hidden">
                  <div className="bg-white/5 px-3 py-2 text-[13px] text-zinc-300">
                    1) แตะปุ่มเมนูมุมขวาบน
                  </div>
                  <img
                    src="/guide-tiktok-step1.png"
                    alt="ขั้นที่ 1: แตะเมนูมุมขวาบน"
                ...
                <li className="rounded-xl border border-white/10 overflow-hidden">
                  ...
                  <img
                    src="/guide-tiktok-step2.png"
                    alt="ขั้นที่ 2: เลือก เปิดในเบราว์เซอร์"
                    className="w-full h-auto block"
                    loading="lazy"
                    decoding="async"
                  />
              ...
              <div className="pt-1">
                <button
                  onClick={onClose}
                  className="inline-flex h-10 items-center rounded-lg px-4 text-[14px] border border-white/10 bg-white/5 text-zinc-200 hover:bg-white/10"
                >
                  ปิด
                </button>
              </div>
        ...
        {loading ? (
          ...
          <div className={gridClass}>
            {items.map((item) => {
              ...
              if (cardMode === "compact") {
                return (
                  <motion.div variants={cardVar(8)} key={item._id}>
              ...
              /* -------------------------- FULL CARD --------------------------- */
              return (
                <motion.div variants={cardVar(10)} key={item._id}>
                  <Card className="overflow-hidden relative group">
                    <div
                    ...
                    >
                      ...
                      {item.productGif && !canHover && (
                        <MobileGifHint playing={gif} onToggle={() => toggleGif(item._id)} placement="br" />
                      )}
                      ...
                      <ScoreBadge value={item.rating || 0} className="absolute top-2 right-2 z-10" />
        ...
        <div className="mx-auto max-w-6xl px-4 md:px-6 py-10 grid gap-10 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <section className="space-y-3">
            ...
            <p className="text-[15px] md:text-[16.5px] leading-[1.9] text-[var(--text-tertiary)]">
              เว็บรวมของผมที่สั่งเอง ใช้เอง รีวิวเอง ความเห็นทั้งหมดมาจากผมเอง ของบางส่วนเป็น Affiliate
              ซึ่งช่วยซัพพอร์ตคอนเทนต์ของผมโดยราคาของจะไม่ได้แพงขึ้น และ
              <span className="font-semibold text-amber-400 ml-1">อย่าลืม!</span>{" "}
              ตรวจสอบสเปก, ราคา, สต็อกให้รอบคอบก่อนซื้อ เว็บไซต์ไม่รับผิดชอบความเสียหายใด ๆ หากของไม่ตรงปกเพราะผมสั่งผมก็เจอไม่ตรงเยอะ ;w;
            </p>
        ...
        <div className="border-t border-[var(--border-subtle)]/80">
          <div className="mx-auto max-w-6xl px-4 md:px-6 py-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <span className="text-[13px] md:text-[14px] text-[var(--text-tertiary)]">
              © {new Date().getFullYear()} ikkist's items — All rights reserved.
            </span>

            <div className="flex items-center gap-3 text-[13px] md:text-[14px] text-[var(--text-tertiary)]">
              <span>
                Built by{" "}
              ...
              {/* ✅ ลิงก์ช่วยเหลือ (สุภาพ ไม่เด่น แต่หาเจอ) */}
              <button
                onClick={() => setGuideOpen(true)}
                className="inline-flex items-center gap-1 text-zinc-400 hover:text-zinc-200 underline underline-offset-2"
                title="หากกดแล้วสินค้าไม่ขึ้น"
              >
                เจอปัญหาสินค้าไม่ขึ้น?
              </button>
            </div>
          </div>
        </div>
